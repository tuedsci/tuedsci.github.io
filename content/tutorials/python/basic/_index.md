---
title: Basic Python course
layout: page
---

### Getting started

- [Welcome](welcome)
- [Introduction to Python](intro-python)
- [Setting up your environment](setup)
- [Introduction to Jupyter Lab](jupyter-lab)
- [Introduction to Markdown](markdown)

### The basics

- [Basic syntax](basic-syntax)
- [Assignments and variables](assignments-variables)
- [Naming rules and conventions](naming)

### Data types

- [Python data types](data-types)
- Built-in scalar types
  - [None](none)
  - [Booleans](booleans)
  - [Ints](ints)
  - [Floats](floats)
- Built-in collection types
  - [Lists (basic)](lists-basic)
  - [Lists (advanced)](lists-advanced)
  - [Tuples](tuples)
  - [Ranges](ranges)
  - [Strings](strings)
  - [Dictionaries](dicts)
  - [Sets](sets)

### Control flows

- [Introduction to control flows](intro-control-flows)
- [Branching](branching)
- [Looping](looping)

### Functions

- [Function fundamentals](func-fundamentals)
- [Default arguments](defaul-args)
- [Anonymous functions](anonymous-func)

### Modules

- [Introduction to modules](intro-modules)
- [Writing a module](writing-module)

### Files

- [Introduction to files](intro-files)
- [Working with files](working-with-files)
